...
public interface ProjectMemberRepository extends JpaRepository<ProjectMember, Long>, ProjectMemberRepositoryCustom {
    ...
    @Query("SELECT COUNT(pm) from ProjectMember pm WHERE pm.projectMemberAuth.id != 4L AND pm.status = 'PARTICIPATING' AND pm.project = :project")
    int countVotableProjectMember(Project project);

    @Query("SELECT COUNT(pm) from ProjectMember pm WHERE pm.id != :fwMemberId AND pm.status = 'PARTICIPATING' AND pm.project.id = :projectId")
    int countFWVotableProjectMember(Long fwMemberId, Long projectId);

    @Query("SELECT pm from ProjectMember pm WHERE pm.project.id = :projectId AND pm.user.id = :userId AND pm.status = 'PARTICIPATING'")
