...
public class TechnologyStackRepositoryImpl implements TechnologyStackRepositoryCustom {
    ...
    public List<TechnologyStack> findTechnologyStackListByIds(List<Long> techStackIds) {
        ...
        List<TechnologyStack> technologyStackList = jpaQueryFactory
    ...
    private void validateTechnologyStackExistence(List<Long> techStackIds, List<TechnologyStack> foundTechnologyStacks) {
        List<Long> foundIds = foundTechnologyStacks.stream()
                .map(TechnologyStack::getId)
                .collect(Collectors.toList());

        List<Long> notFoundIds = techStackIds.stream()
