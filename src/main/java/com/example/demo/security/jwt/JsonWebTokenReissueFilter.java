...
public class JsonWebTokenReissueFilter extends OncePerRequestFilter {
    ...
    // 토큰 재발급 로직
    private void handleTokenReissue(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        try {
    ...
    private Long parseUserId(HttpServletRequest request) throws IOException {
        try {
            UserTokenReissueRequestDto tokenReissueRequestData =
                    objectMapper.readValue(
                            request.getInputStream(), UserTokenReissueRequestDto.class);

            // Request 정보가 존재하지 않는 경우, 토큰 재발급을 위한 회원 식별 정보 부족 예외처리
            if (Objects.isNull(tokenReissueRequestData)
                    || tokenReissueRequestData.getUserId() == null) {
    ...
    private void successTokenReissueResponse(HttpServletResponse response) throws IOException {
        securityResponseHandler.sendResponse(
                response, HttpStatus.OK, ResponseDto.success("토큰 재발급이 완료되었습니다."));
    }
