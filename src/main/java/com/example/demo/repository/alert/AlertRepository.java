...
public interface AlertRepository extends JpaRepository<Alert, Long> {
    ...
    @Query(
            "select alert from Alert alert where alert.project = :#{#project.id} and alert.type = 'RECRUIT'")
    Optional<List<Alert>> findRecruitAlertsByProject(@Param(value = "project") Project project);

    @Query(
            "select alert from Alert alert where alert.project = :#{#project.id} and alert.type = 'WORK'")
    Optional<List<Alert>> findWorkAlertsByProject(@Param(value = "project") Project project);

    @Query(
            "select alert from Alert alert where alert.project = :#{#project.id} and alert.type = 'ADD' or alert.type = 'WITHDRAWL' or alert.type = 'FORCEDWITHDRAWL'")
